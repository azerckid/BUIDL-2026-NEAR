...
2026-06-01 01:57 KST 기준 한화손보 다이렉트 내가고른 암보험 공시 adapter를 보강했다. `landing.do` 경로는 세션/브라우저 조건이 없으면 error page로 이동할 수 있어, 공식 화면의 `/resource/inspl/ltr/cncr/js/main.js`에서 약관 다운로드 버튼이 호출하는 `/clapdf/LA02969001.pdf` 경로를 재현했다. 신규 SHA-256은 `ca8dd26a25c1aa60cefb4c298c8df843f8a35d5bf0ff758a0624e37ddaf15ca0`이고 PDF size는 2,071,737 bytes다. 이번 단계는 DB write 없이 crawler/data/docs만 변경했으며, 한화손보 암보험 source는 아직 `raw`라 추천 snapshot 수는 15개로 유지한다. 검증은 `../05_QA_Validation/77_HANWHA_GENERAL_CANCER_DISCLOSURE_ADAPTER_PROBE_2026_06_01.md`에 둔다. 다음 작업은 한화손보 암보험 문서 variant와 매칭 키워드/caveat를 정리하는 것이다.
...
2026-06-01 04:30 KST 기준 롯데손보 실손의료보험의 문서 variant와 baseline 매칭 키워드/caveat를 정리했다. 공식 약관 1건과 보험다모아 숫자 quote 4건이 있어 `coverage_category=medical_expense`, `matching_strategy=baseline`, `risk_targets=[]` 기준의 baseline-ready 후보로 볼 수 있다. 다만 상품요약서와 사업방법서 hash는 아직 확보하지 않았으므로 후속 seed PR에서 이 caveat를 유지한다. 검증은 `../05_QA_Validation/86_LOTTE_MEDICAL_MATCHING_REVIEW_2026_06_01.md`에 둔다. 다음 작업은 롯데손보 source document seed, quote approval, baseline snapshot seed PR이다.

2026-06-01 04:59 KST 기준 롯데손보 실손 baseline 추천 snapshot seed 준비를 완료했다. `seed.ts`는 적용 시 source document 1건을 추가하고, 롯데손보 source를 `approved`로 승격하며, 보험다모아 실손 quote 4건을 `approved`로 바꾸고, `prod_lotte_direct_medical_202605` snapshot 1건을 추가한다. 이번 단계는 운영 DB write 없이 seed/data/docs만 변경하며, 검증은 `../05_QA_Validation/87_LOTTE_MEDICAL_BASELINE_SNAPSHOT_SEED_2026_06_01.md`에 둔다. 다음 작업은 운영 DB 백업 후 seed apply PR이다.
