...
2026-05-28 quote matrix PoC 이후 `src/lib/db/schema.ts`와 `drizzle/0006_real_war_machine.sql`에 schema/migration을 추가했다. 운영 Turso DB에는 2026-05-28 20:55 KST 기준 migration을 적용했고, 21:12 KST 기준 16건, 23:50 KST 기준 실손 여성 quote 8건, 2026-05-29 01:20 KST 기준 quote-only raw source 기반 60건을 추가해 quote row 총 84건을 `needs_review` 상태로 적재했다.
...
6. [x] seed 교체 전, raw source seed와 recommendation snapshot seed를 분리한다.
