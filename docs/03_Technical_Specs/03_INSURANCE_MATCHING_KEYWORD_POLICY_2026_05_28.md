...
다음 단계는 동양생명 암보험 매칭 키워드/caveat 정리, 삼성생명 입원 건강보험 category 정책 결정, 신한라이프 표준형 일반형 endpoint blocker 유지/재탐색을 순차 처리하는 것이다. 한화손보 실손은 갱신형 V 공식 문서 endpoint 발견 전까지 blocker로 유지한다. 아직 source 후보로 구조화하지 못한 보험다모아 P0 샘플 34개는 공식 URL, source row, 문서 hash 순서로 별도 확장한다.
