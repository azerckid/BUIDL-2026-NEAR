...
다음 단계는 남은 raw blocker 2건을 정리하는 것이다. 신한라이프 표준형은 일반형 공식 문서 endpoint 발견 전까지 blocker로 유지하고, 한화손보 실손은 갱신형 V 공식 문서 endpoint 발견 전까지 blocker로 유지한다. 아직 source 후보로 구조화하지 못한 보험다모아 P0 샘플 34개는 공식 URL, source row, 문서 hash 순서로 별도 확장한다.
