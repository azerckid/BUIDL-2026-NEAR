...
2026-06-01 03:34 KST 기준 DB생명 e로운 암보험을 `coverage_category=oncology`, `matching_strategy=risk_target`, 공통 5개 암 risk target 기준의 snapshot 후보로 정리했다. 약관은 90일 암 보장개시일, 1년 미만 50% 감액, 유방암 180일 이전 20% 지급, 소액암/특정3대암 별도 급부, 해약환급금 미지급형 caveat를 명시한다. 숫자 KRW quote 4건이 있으므로 다음 seed PR에서 source document 1건, quote approval 4건, active `insurance_products` snapshot 1건을 준비할 수 있다. 검증은 `../05_QA_Validation/82_DB_LIFE_CANCER_MATCHING_REVIEW_2026_06_01.md`에 둔다.
...
2026-06-01 04:30 KST 기준 롯데손보 실손의료보험을 `coverage_category=medical_expense`, `matching_strategy=baseline`, `risk_targets=[]` 기준의 baseline-ready 후보로 정리했다. 약관은 공식 상품 페이지의 `약관보기` 버튼에서 제공되는 `let_care_sil_2605_yak.pdf`이며, 보험다모아 숫자 quote 4건도 확인됐다. 다만 상품요약서와 사업방법서 hash는 아직 확보하지 않았으므로 후속 seed PR에서 문서 범위 caveat를 유지한다. 검증은 `../05_QA_Validation/86_LOTTE_MEDICAL_MATCHING_REVIEW_2026_06_01.md`에 둔다.
...
2026-06-01 12:14 KST 기준 한화손보 실손의료보험 source는 공식 페이지와 PDF를 찾았지만 target source와 variant가 달라 추천 snapshot 후보에서 제외했다. 공식 페이지/PDF는 `갱신형 III`로 식별되고 target은 `갱신형 V`이므로, 실손 세대/개정 오연결을 막기 위해 source document seed, quote approval, product snapshot 발행을 모두 보류한다. 검증은 `../05_QA_Validation/89_HANWHA_GENERAL_MEDICAL_DISCLOSURE_PROBE_2026_06_01.md`에 둔다.

2026-06-01 12:51 KST 기준 동양생명 `무배당우리WON하는실속하나로암보험`의 공식 문서 hash 3건을 확보했다. 상품요약서, 사업방법서, 보험약관 모두 공시실 `2026.03.01` row의 `MasFiledownload` POST endpoint에서 계산했다. 아직 `coverage_category`, `risk_targets`, `coverage_caveats_json`, quote approval을 정리하지 않았으므로 source는 `raw`로 유지한다. 검증은 `../05_QA_Validation/90_TONGYANG_LIFE_CANCER_DISCLOSURE_ADAPTER_PROBE_2026_06_01.md`에 둔다.

2026-06-01 13:06 KST 기준 동양생명 암보험을 `coverage_category=oncology`, `matching_strategy=risk_target`, `risk_targets=[pancreatic_cancer,liver_cancer,lung_cancer,breast_cancer,colon_cancer]` 기준의 snapshot 후보로 정리했다. caveat는 90일 암 보장개시일, 1년 미만 50% 감액, 유방암 180일 10% 조건, 소액암/갑상선/피부암 분류, 납입면제 제외, 갱신형 특약 보험료 변동, 가입/건강진단 조건, 해약환급금 유의사항으로 정리한다. 검증은 `../05_QA_Validation/91_TONGYANG_LIFE_CANCER_MATCHING_REVIEW_2026_06_01.md`에 둔다.
...
2026-06-01 15:05 KST 기준 삼성생명 인터넷 입원 건강보험은 현재 추천 카테고리 밖으로 판단했다. 이 상품은 공식 통합약관 hash와 대표 보험료는 있으나 `oncology`, `cardiovascular`, `metabolic`, `neurological`, `medical_expense` 중 어느 것도 자연스러운 매칭이 아니며, 조건별 quote matrix도 없다. 따라서 `hospitalization` 또는 `general_health` 카테고리 정책이 별도 schema/i18n/UI PR로 추가되기 전까지 `review_status=rejected`, source catalog only로 처리한다. 검증은 `../05_QA_Validation/94_SAMSUNG_LIFE_HOSPITAL_HEALTH_POLICY_2026_06_01.md`에 둔다.

2026-06-01 15:32 KST 기준 삼성생명 입원 건강보험 source catalog exclusion을 운영 DB에 적용했다. 적용 후 source는 `rejected`, active 추천 상품은 19건, approved quote는 76건, 남은 raw blocker는 2건이다. 검증은 `../05_QA_Validation/95_SAMSUNG_LIFE_HOSPITAL_HEALTH_DB_APPLY_2026_06_01.md`에 둔다.
...
2026-06-01 19:26 KST 기준 신한라이프 표준형 암보험 source는 최신 endpoint 재탐색에서도 일반형 문서 row가 없으므로 추천 후보에서 종결 제외한다. source는 `rejected`, quote 4건은 `rejected`로 내리는 seed 변경을 준비했다. no-refund 문서 3건은 `src_shinhan_life_sol_cancer_202601` 전용 근거로 유지하고 표준형 source에 재사용하지 않는다. 검증은 `../05_QA_Validation/98_SHINHAN_STANDARD_BLOCKER_POLICY_2026_06_01.md`에 둔다.

2026-06-01 19:38 KST 기준 신한라이프 표준형 blocker seed를 운영 DB에 적용했다. 적용 후 현재 source catalog 후보 22개는 `approved=19`, `rejected=3`, `raw=0`, `needs_review=0`이다. active 추천 상품은 19건, approved quote는 76건으로 유지된다. 검증은 `../05_QA_Validation/99_SHINHAN_STANDARD_BLOCKER_DB_APPLY_2026_06_01.md`에 둔다.

다음 단계는 한화생명 e-insmarket NULL quote 4건을 별도 hygiene PR로 정리하는 것이다. 아직 source 후보로 구조화하지 못한 보험다모아 P0 샘플 34개는 공식 URL, source row, 문서 hash 순서로 별도 확장한다.
...
- **QA_Validation**: [Hanwha General Medical Disclosure Probe](../05_QA_Validation/89_HANWHA_GENERAL_MEDICAL_DISCLOSURE_PROBE_2026_06_01.md) - 한화손보 실손 official PDF variant mismatch 차단 검증
