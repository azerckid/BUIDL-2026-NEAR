...
**The Secret Keeper** (코드명: MyGenome Agent)는 OHmyDNA의 **사용자 편의 상담 레이어**입니다. TEE 분석 및 ZKP 파이프라인이라는 핵심 프라이버시 인프라 위에 얹히는 부가 기능으로, NEAR AI Cloud를 활용하여 사용자가 자신의 유전적 위험 레이블을 기반으로 보험 관련 질문을 할 수 있도록 돕습니다.

본 에이전트는 **"과거를 기억하지 않고, 현재의 고통(질병)에만 집중한다"**는 **Stateless & Memory-less** 원칙을 따릅니다. 사용자의 원본 유전자 데이터는 TEE 분석 즉시 소각(Purge)되며, 에이전트는 사용자의 DNA를 '알지 못하는' 상태에서 사용자가 입력한 현재의 건강 고민이나 질병 증상에 대해 전문가급 상담을 수행하고, 최적의 보험 상품을 비밀스럽게 연결합니다.
...
### 4-1. 데이터 격리 및 망각 정책 (Data Isolation & Oblivion)

*   **Zero-Copy & Immediate Purge**: 유전자 필터링 분석 결과(Label)는 대화 세션 동안에만 TEE 메모리에 임시 상주하며, **상담 종료 또는 브라우저 종료 시 TEE 및 서버 메모리에서 즉시 영구 소각**됩니다.
*   **Stateless Inference**: 에이전트는 사용자의 DNA 원본 시퀀스를 절대 저장하거나 학습하지 않습니다. 매 상담은 독립적인 세션으로 취급되며, "에이전트는 사용자의 유전자를 모른다"는 상태를 유지합니다.
*   **Encrypted History**: 대화 이력은 사용자의 NEAR Wallet 개인키로 암호화되어 DB에 저장됩니다. 서버 관리자나 AI 모델조차 사용자의 명시적 복호화 없이는 내용을 읽을 수 없습니다.
