...
> Last Updated: 2026-05-27 15:36

- **레이어**: 05_QA_Validation
- **상태**: Review Queue v1 완료
- **범위**: 보험다모아 P0 상품 row, 공식 상품 페이지 probe, 보험사 공시실 crawler 결과를 결합해 사람이 검수할 CSV 생성
- **결론**: 총 56개 상품 row 중 공식 상품 URL이 있는 row는 47개이고, 공식 문서 hash가 확보된 검수 우선 row는 3개다. 이 CSV는 서비스 DB seed가 아니라 사람 검수 대기열이다.
...
| hash 문서 수 | 6개 |
| `needs_human_review` | 3개 |
| `needs_source_document` | 44개 |
...
1. 위 3개 hash-backed row의 수동 검수 결과는 `08_HASH_BACKED_PRODUCT_MANUAL_REVIEW_2026_05_27.md`에 기록한다.
2. 한화생명 e암보험은 출처 기반 카탈로그 후보로 남기되, 현재 `insurance_products` seed에는 바로 넣지 않는다.
3. DB손보/삼성화재 실손의료보험은 `medical_expense` 카테고리 또는 별도 원천 상품 테이블이 생긴 뒤에 승격한다.
3. `needs_source_document` 44개는 보험사별 JS/API adapter를 추가해 문서 hash를 확보한다.
4. `needs_official_product_url` 9개는 보험다모아 row의 공식 이동 URL 추출 실패 원인을 재확인한다.
...
- **QA_Validation**: [Carrier Disclosure Crawler](./06_CARRIER_DISCLOSURE_CRAWLER_2026_05_27.md) - 공시실 crawler와 DB손보 API adapter 검증 결과
- **QA_Validation**: [Hash-backed Product Manual Review](./08_HASH_BACKED_PRODUCT_MANUAL_REVIEW_2026_05_27.md) - hash-backed 3개 상품의 수동 검수 결과
