...
- **상태**: Manual Review v1 완료
- **범위**: `latest_insurance_review_queue.csv`에서 공식 문서 hash가 확보된 3개 상품
- **결론**: 현재 `insurance_products` 스키마에 바로 넣을 수 있는 상품은 0개다. 한화생명 e암보험은 출처 기반 카탈로그 후보로 승격 가능하지만, DB손보/삼성화재 실손의료보험은 `medical_expense` 계열 스키마가 먼저 필요하다.
...
- **QA_Validation**: [Carrier Disclosure Crawler](./06_CARRIER_DISCLOSURE_CRAWLER_2026_05_27.md) - 삼성화재/DB손보 문서 hash 확보 결과
