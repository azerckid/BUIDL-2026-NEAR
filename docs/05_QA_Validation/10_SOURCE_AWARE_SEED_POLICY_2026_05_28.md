...
- **상태**: Seed Policy QA v1 완료
- **범위**: `src/lib/db/seed.ts`가 공식 보험상품 후보를 어떤 테이블에 넣고, 사용자 추천 노출을 어떻게 차단하는지 검증한다.
- **결론**: hash-backed 7개 상품은 실제 추천 상품으로 활성화하지 않고, source-aware 검수 후보로만 seed에 반영한다. 기존 active demo 상품 5개는 서비스 흐름 보존용으로 유지한다.
...
| 수동 검수 입력 | `data/insurance/latest_seed_candidate_review.json` |
| 보험료 정책 | `docs/04_Logic_Progress/04_INSURANCE_PREMIUM_QUOTE_POLICY_2026_05_28.md` |
| 로드맵 연결 | `docs/04_Logic_Progress/ROADMAP.md` |

이번 검증에서는 seed 스크립트를 실제 DB에 실행하지 않았다. 운영 또는 로컬 Turso DB에 데이터를 쓰기 전에는 대상 DB, 백업 여부, 환경 변수를 별도로 확인해야 한다.
...
실제 상품 후보를 `insurance_products`에 넣지 않은 이유는 다음과 같다.

- 수동 검수 결과 `current_insurance_products_seed_ready_count=0`이다.
- 한화생명 후보의 `premium_text=0원`은 대표 월 보험료로 사용할 수 없다.
- 암보험 후보는 암 급부 차이, 면책기간, 감액기간 caveat가 아직 승인되지 않았다.
...
| `monthly_premium_usdc` | 실제 후보에는 적용하지 않음. active 추천 상품 승격 시 별도 환산 기준 필요 |
...
| source 후보가 많아져도 추천 엔진이 읽지 않음 | 승인된 상품만 별도 PR에서 `insurance_products` snapshot으로 승격 |
...
1. 로컬 Turso DB에서 seed dry-run 또는 제한 실행을 수행하고 row count를 확인한다.
2. 보험다모아/보험사 페이지에서 나이와 성별 파라미터 재조회 가능성을 PoC로 확인한다.
3. `insurance_premium_quotes` schema/migration 초안을 작성한다.
4. 승인된 실제 상품만 `insurance_products` active snapshot으로 승격하고 기존 demo 상품 제거 시점을 정한다.
...
| Rubric | 검증 의미 |
|---|---|
| Functionality | 공식 출처 후보를 DB seed 경로에 올리되 승인 전 추천 노출을 차단한다 |
| Potential Impact | 실제 보험상품 카탈로그를 운영 가능한 검수 파이프라인으로 전환한다 |
| Novelty | PDF hash 기반 공식 출처와 유전자 위험 매칭의 경계를 분리한다 |
| UX | 사용자는 아직 승인되지 않은 보험료를 확정 견적으로 오해하지 않는다 |
| Open-source | 다른 빌더가 source row와 active 추천 row를 분리하는 패턴을 재사용할 수 있다 |
| Business Plan | 보험 비교/중개 서비스로 가기 위한 판매상태, 가격, caveat 승인 단계를 명확히 한다 |
...
- **QA_Validation**: [Hash-backed Product Manual Review](./08_HASH_BACKED_PRODUCT_MANUAL_REVIEW_2026_05_27.md) - 7개 상품 수동 검수와 seed 차단 근거
