...
- **상태**: Draft v1.0 — **Phase 2 구현 완료 후 실행 예정. 현재 시점에서는 실행 불가.**

---

> **주의**: 이 문서의 모든 시나리오는 Phase 2에서 구현될 AI 상담 레이어를 대상으로 합니다.
> Phase 0(해커톤 데모) 단계에서는 채팅 UI, 대화 이력 DB, TEE memory_wipe 함수가 미구현 상태이므로
> 아래 검증 항목을 현재 코드베이스에서 실행할 수 없습니다.
