...
- **작성일**: 2026-03-31
- **최종 수정일**: 2026-05-28 (보험료 quote matrix PoC)
...
| 트랙 | 핵심 질문 | 다음 작업 |
|---|---|---|
| 실제 보험상품 카탈로그 | 실제 판매 상품과 조건별 보험료를 어떤 공식 출처로 검증하고 주기적으로 갱신할 것인가 | 보험료 quote matrix PoC 부분 완료, `insurance_premium_quotes` schema 설계 준비 |
...
2026-05-28 보험료 quote matrix PoC에서는 보험다모아 모바일 출처로 8개 source probe와 66개 quote row를 재조회했다. 암보험은 34세/44세 남성/여성 조건이 모두 조회됐고, 실손의료보험은 남성 34세/44세 조건에서 DB손보, KB손보, 삼성화재, 현대해상 상품의 보험료 변동이 확인됐다. 실손의료보험 여성 조건은 현재 `sexDiv=F`, `sex=F` 파라미터로 HTTP 500을 반환하므로, 다음 작업은 `insurance_premium_quotes` schema/migration 설계와 실손 여성 파라미터 확인이다. 검증 결과는 `../05_QA_Validation/12_PREMIUM_QUOTE_MATRIX_POC_2026_05_28.md`, 산출물은 `../../data/insurance/latest_premium_quote_probe.json`에 기록한다.
