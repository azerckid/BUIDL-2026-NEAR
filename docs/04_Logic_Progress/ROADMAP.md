...
> Last Updated: 2026-06-01 19:38

- **작성일**: 2026-03-31
- **최종 수정일**: 2026-06-01 (신한라이프 표준형 blocker DB 적용)
...
여기서 현재 추천 상품 19개는 “수집한 전체 데이터 수”가 아니라 “사용자 추천 화면에 노출 가능한 최종 snapshot 수”다. 현재까지 확보한 기반 데이터는 보험다모아 P0 샘플 56개, source catalog 후보 22개, 공식 문서 row 39개, 조건별 보험료 quote row 92개이며, 이 중 원천 근거, 매칭 키워드, caveat, approved quote를 통과해 active 추천으로 발행된 상품이 19개다. 동양생명까지 적용된 조건별 보험료 approved row는 76건이다. 구조화된 source 후보 22개는 `approved=19`, `rejected=3`, `raw=0`, `needs_review=0`으로 닫혔다. 전체 quote 기준으로는 한화생명 e-insmarket NULL quote 4건이 아직 `needs_review`라 별도 hygiene PR이 필요하다.
...
| 6 | 보험상품 확장 | 남은 non-approved source의 문서 hash, 매칭 키워드, caveat 정리 | source 후보 22개 approved/rejected 정리 완료. 한화생명 NULL quote 4건 hygiene 필요 |
| 7 | 추천 snapshot 확대 | 새 source를 `approved`로 승격하고 `insurance_products` snapshot 발행 | 운영 active 추천 19건, approved quote 76건 |
...
2026-05-31 19:49 KST 기준 농협손보 실손 baseline 추천 snapshot seed 준비를 완료했다. `seed.ts`는 적용 시 `doc_nh_fire_medical_terms_202605` source document 1건을 추가하고, 농협손보 source를 `approved`로 승격하며, quote 4건을 `approved`로 바꾸고, `prod_nh_fire_medical_202605` snapshot 1건을 추가한다. 운영 DB 읽기 전용 확인 결과 현재 농협손보 source는 `raw`, quote 4건은 `needs_review`, source document는 0건이다. 이번 단계는 DB write 없이 seed/data/docs만 변경하며, 검증은 `../05_QA_Validation/63_NH_FIRE_BASELINE_SNAPSHOT_SEED_2026_05_31.md`에 기록한다. 다음 작업은 운영 DB 백업 후 seed apply PR이다. 적용 완료 후 source-backed active 추천 상품은 10개에서 11개로 늘어난다.
...
2026-05-31 21:52 KST 기준 메리츠화재 실손 baseline 추천 snapshot seed 준비를 완료했다. `seed.ts`는 적용 시 source document 3건을 추가하고, 메리츠화재 source를 `approved`로 승격하며, quote 4건을 `approved`로 바꾸고, `prod_meritz_direct_medical_202605` snapshot 1건을 추가한다. 운영 DB 읽기 전용 확인 결과 현재 메리츠화재 source는 `raw`, quote 4건은 `needs_review`, source document는 0건이다. 이번 단계는 DB write 없이 seed/data/docs만 변경하며, 검증은 `../05_QA_Validation/67_MERITZ_FIRE_BASELINE_SNAPSHOT_SEED_2026_05_31.md`에 기록한다. 다음 작업은 운영 DB 백업 후 seed apply PR이다. 적용 완료 후 source-backed active 추천 상품은 11개에서 12개로 늘어난다.

2026-05-31 22:01 KST 기준 메리츠화재 실손 baseline 추천 snapshot을 운영 DB에 백업 후 적용했다. 적용 후 `insurance_source_documents=26`, `insurance_products=17`, source-backed active 추천 상품은 12건이 됐고, `insurance_product_sources.review_status=approved`는 12건, `insurance_premium_quotes.review_status=approved`는 48건으로 증가했다. 메리츠화재 source 1건, document 3건, quote 4건, product snapshot 1건이 모두 approved/active 상태다. 검증은 `../05_QA_Validation/68_MERITZ_FIRE_BASELINE_DB_APPLY_2026_05_31.md`에 기록한다. 다음 작업은 Dashboard와 상담 AI에서 메리츠화재 카드 설명을 수동 확인하거나, 흥국화재/미래에셋생명/한화손보 adapter를 순차 보강하는 것이다.
...
2026-06-01 00:48 KST 기준 미래에셋생명 온라인 암보험 2건의 추천 snapshot seed 준비를 완료했다. `seed.ts`는 적용 시 source document 6건, source approval 2건, quote approval 8건, `insurance_products` snapshot 2건을 추가한다. 이번 단계는 운영 DB write 없이 seed/data/docs만 변경했으며, 적용 완료 후 source-backed active 추천 상품은 13건에서 15건, approved quote는 52건에서 60건으로 늘어난다. 검증은 `../05_QA_Validation/75_MIRAEASSET_LIFE_CANCER_SNAPSHOT_SEED_2026_06_01.md`에 기록한다. 다음 작업은 운영 DB 백업 후 seed apply PR이다.
...
2026-06-01 12:51 KST 기준 동양생명 `무배당우리WON하는실속하나로암보험` 공식 공시실 adapter probe를 완료했다. 공시실 판매상품 row는 `2026.03.01` 기준이고, `MasFiledownload` POST 다운로드로 상품요약서, 사업방법서, 보험약관 3건의 SHA-256 hash를 확보했다. 운영 DB write는 없고 active 추천 상품은 18건으로 유지된다. 검증은 `../05_QA_Validation/90_TONGYANG_LIFE_CANCER_DISCLOSURE_ADAPTER_PROBE_2026_06_01.md`에 기록한다. 다음 작업은 동양생명 암보험의 매칭 키워드/caveat 정리다.

2026-06-01 13:06 KST 기준 동양생명 암보험의 매칭 키워드/caveat 정리를 완료했다. 공식 문서 3건과 숫자 KRW quote 4건이 있어 `coverage_category=oncology`, `matching_strategy=risk_target`, 암 관련 risk target 5개 기준의 snapshot 후보로 판단한다. 운영 DB write는 없고 active 추천 상품은 18건으로 유지된다. 검증은 `../05_QA_Validation/91_TONGYANG_LIFE_CANCER_MATCHING_REVIEW_2026_06_01.md`에 기록한다. 다음 작업은 동양생명 source document seed 3건, quote approval 4건, active oncology product snapshot 1건을 준비하는 seed PR이다.

2026-06-01 13:20 KST 기준 동양생명 암보험 추천 snapshot seed 준비를 완료했다. 운영 DB 읽기 전용 확인 결과 source는 `raw`, document 0건, product 0건, quote 4건은 `needs_review` 상태였다. `seed.ts`는 적용 시 source document 3건을 추가하고, 동양생명 source를 `approved`로 승격하며, quote 4건을 `approved`로 바꾸고, `prod_tongyang_wooriwon_cancer_202605` active oncology snapshot 1건을 추가한다. 이번 단계는 DB write 없이 seed/data/docs만 변경하며, 검증은 `../05_QA_Validation/92_TONGYANG_LIFE_CANCER_SNAPSHOT_SEED_2026_06_01.md`에 기록한다. 다음 작업은 운영 DB 백업 후 seed apply PR이다. 적용 후 source-backed active 추천 상품은 18건에서 19건, approved quote는 72건에서 76건으로 늘어나야 한다.

2026-06-01 13:42 KST 기준 운영 DB 백업 후 동양생명 암보험 추천 snapshot을 적용했다. 적용 후 `insurance_source_documents=39`, `insurance_products=24`, source-backed active 추천 상품은 19건, oncology active 상품은 11건, approved quote는 76건이다. 동양생명 source 1건, document 3건, quote 4건, product snapshot 1건이 모두 approved/active 상태다. 검증은 `../05_QA_Validation/93_TONGYANG_LIFE_CANCER_DB_APPLY_2026_06_01.md`에 기록한다. 다음 작업은 Dashboard와 상담 AI에서 동양생명 카드 설명을 수동 확인하거나, 남은 non-approved source 3건의 blocker를 순차 처리하는 것이다.

2026-06-01 15:05 KST 기준 삼성생명 인터넷 입원 건강보험 category 정책을 결정했다. 현재 추천 엔진과 `insurance_products.coverage_category` enum은 DNA risk target 4개 계열과 `medical_expense` baseline만 지원하므로, 입원 건강보험을 기존 카테고리에 억지로 넣어 active 추천으로 발행하지 않는다. `seed.ts`에는 이 source를 `review_status=rejected`로 내리는 source catalog exclusion update를 추가했으며, DB write는 후속 apply PR로 분리한다. active 추천 상품 수와 approved quote 수는 변하지 않는다. 검증은 `../05_QA_Validation/94_SAMSUNG_LIFE_HOSPITAL_HEALTH_POLICY_2026_06_01.md`에 기록한다. 다음 작업은 운영 DB 백업 후 exclusion apply PR이거나 한화손보 실손/신한라이프 표준형 raw blocker 재탐색이다.

2026-06-01 15:32 KST 기준 운영 DB 백업 후 삼성생명 입원 건강보험 source catalog exclusion을 적용했다. 적용 후 해당 source는 `review_status=rejected`, source-backed active 추천 상품은 19건, approved quote는 76건으로 유지된다. 남은 raw blocker는 한화손보 실손과 신한라이프 표준형 2건이다. 검증은 `../05_QA_Validation/95_SAMSUNG_LIFE_HOSPITAL_HEALTH_DB_APPLY_2026_06_01.md`에 기록한다. 다음 작업은 한화손보 실손 `갱신형 V` endpoint 재탐색/종결 정책 또는 신한라이프 표준형 일반형 endpoint 재탐색/종결 정책이다.

2026-06-01 15:40 KST 기준 한화손보 실손의료보험 `갱신형 V` blocker 종결 정책을 seed에 준비했다. 공식 후보 페이지/PDF는 접근 가능하지만 `갱신형 III` 계열이라 target source와 맞지 않으므로 source를 `rejected`로 내리고 quote 4건도 `rejected`로 내린다. DB write는 후속 apply PR로 분리한다. 적용 후 active 추천 상품 19건은 유지되고, 남은 raw blocker는 신한라이프 표준형 1건이어야 한다. 검증은 `../05_QA_Validation/96_HANWHA_GENERAL_MEDICAL_BLOCKER_POLICY_2026_06_01.md`에 기록한다.

2026-06-01 19:13 KST 기준 운영 DB 백업 후 한화손보 실손 blocker를 적용했다. 적용 후 source는 `review_status=rejected`, quote 4건은 `review_status=rejected`가 됐고, source-backed active 추천 상품은 19건, approved quote는 76건으로 유지된다. 남은 raw blocker는 신한라이프 표준형 1건이다. 검증은 `../05_QA_Validation/97_HANWHA_GENERAL_MEDICAL_BLOCKER_DB_APPLY_2026_06_01.md`에 기록한다.

2026-06-01 19:26 KST 기준 신한라이프 표준형 암보험 endpoint probe를 재실행했고, 표준형 일반 문서 row는 여전히 발견되지 않았다. 반환되는 신한SOL암보험 row는 해약환급금 미지급형 1건뿐이므로 no-refund 문서 3건은 표준형 source에 재사용하지 않는다. `seed.ts`에는 신한라이프 표준형 source와 quote 4건을 `rejected`로 내리는 blocker 정책을 추가했다. DB write는 후속 apply PR로 분리한다. 검증은 `../05_QA_Validation/98_SHINHAN_STANDARD_BLOCKER_POLICY_2026_06_01.md`에 기록한다.
...
- [메리츠화재 실손 Baseline 추천 Snapshot Seed 검증](../05_QA_Validation/67_MERITZ_FIRE_BASELINE_SNAPSHOT_SEED_2026_05_31.md)
...
- [흥국화재 실손 Baseline 추천 Snapshot DB 적용 검증](../05_QA_Validation/72_HEUNGKUK_FIRE_BASELINE_DB_APPLY_2026_05_31.md)
- [미래에셋생명 공시 Adapter Probe 검증](../05_QA_Validation/73_MIRAEASSET_LIFE_DISCLOSURE_ADAPTER_PROBE_2026_06_01.md)
- [미래에셋생명 온라인 암보험 매칭 검수](../05_QA_Validation/74_MIRAEASSET_LIFE_CANCER_MATCHING_REVIEW_2026_06_01.md)
...
- [DB생명 암보험 추천 Snapshot DB 적용 검증](../05_QA_Validation/84_DB_LIFE_CANCER_DB_APPLY_2026_06_01.md)
- [롯데손보 실손의료보험 공시 Adapter Probe 검증](../05_QA_Validation/85_LOTTE_MEDICAL_DISCLOSURE_ADAPTER_PROBE_2026_06_01.md)
...
- [동양생명 암보험 공시 Adapter Probe 검증](../05_QA_Validation/90_TONGYANG_LIFE_CANCER_DISCLOSURE_ADAPTER_PROBE_2026_06_01.md)
- [동양생명 암보험 매칭 키워드와 Caveat 검수](../05_QA_Validation/91_TONGYANG_LIFE_CANCER_MATCHING_REVIEW_2026_06_01.md)
- [동양생명 암보험 추천 Snapshot Seed 검증](../05_QA_Validation/92_TONGYANG_LIFE_CANCER_SNAPSHOT_SEED_2026_06_01.md)
- [동양생명 암보험 추천 Snapshot DB 적용 검증](../05_QA_Validation/93_TONGYANG_LIFE_CANCER_DB_APPLY_2026_06_01.md)
- [삼성생명 입원 건강보험 Category 정책 검증](../05_QA_Validation/94_SAMSUNG_LIFE_HOSPITAL_HEALTH_POLICY_2026_06_01.md)
- [삼성생명 입원 건강보험 Source Exclusion DB 적용 검증](../05_QA_Validation/95_SAMSUNG_LIFE_HOSPITAL_HEALTH_DB_APPLY_2026_06_01.md)
- [한화손보 실손의료보험 Blocker 종결 정책 검증](../05_QA_Validation/96_HANWHA_GENERAL_MEDICAL_BLOCKER_POLICY_2026_06_01.md)
- [한화손보 실손의료보험 Blocker DB 적용 검증](../05_QA_Validation/97_HANWHA_GENERAL_MEDICAL_BLOCKER_DB_APPLY_2026_06_01.md)
